...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    // ------------------------------------------------------------------
    // API v1 enforces a maximum batch size of 32. Check before sending.
    // ------------------------------------------------------------------
    const batchSize = Math.max(requestLats.length, requestLons.length, requestDates.length);
    if (batchSize > 32) {
      setErrorModal({
        isOpen: true,
        message: `You have selected ${batchSize} profiles which exceeds the maximum allowed (32).\nPlease reduce your selection and try again.`,
      });
      return;
    }

    try {
      // Updated endpoint for the new API version
      ...
      if (response.ok) {
        ...
        // Show success message with metadata from headers (new API)
        const modelSHA = response.headers.get('MODEL_SHA') || 'unknown';
        const statsSHA = response.headers.get('STATS_SHA') || 'unknown';
        const satSnapshot = response.headers.get('SAT_SNAPSHOT') || 'unknown';
        
        setErrorModal({
          isOpen: true,
          message: `Data downloaded successfully!\nModel SHA: ${modelSHA}\nStats SHA: ${statsSHA}\nSatellite snapshot: ${satSnapshot}`
        });
      } else {
        // Handle error response
        const errorData = await response.clone().json().catch(async () => {
          // Fallback to plain text if JSON is not available
